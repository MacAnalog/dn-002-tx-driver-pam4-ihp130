...
This port was step 1 (runnable netlists + validated metric extraction) of
the plan in `spicexplorer-tcas-2026/doc/pam4_driver_case_study_feasibility.md`.
The later steps are now in this repo: the parameterized layout generator and
signoff/PEX chain (`layout/`), the layout-in-the-loop optimization
(`notebooks/02`), full pre/post-layout signoff (`notebooks/03`), and the
layout/schematic co-design run *through the SpiceXplorer platform*
(`layout/codesign/`, `notebooks/04`) that produced the layout of record
(round-3 best-score point `r3_s12/run_26`, the paper's design of record:
post-layout S11 −10.16 / S22 −10.28 dB at the block's halo-8 instrument,
167 mW, 7268 µm²; the round-3 accepted point v4 is kept as `V4_LAYOUT`). The `bias`-port VCCS
convention makes the tail current a plain `.param` knob for the optimizer,
which is how the electrical knobs (nx, tail, R_C, R_B, C_deg, V_casc) enter
the joint search. Reviewer-facing evidence for every claim is regenerated by
`make report` into `report/`.
