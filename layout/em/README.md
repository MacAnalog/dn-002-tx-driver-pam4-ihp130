...
## Round-4 point (`R4_LAYOUT`, `target_r4/`)

The same lane on the co-design round-4 point (`gen_layout.R4_LAYOUT` = r4_s10/run_29
with two input-side knobs reverted after the CM→diff audit, `layout/codesign/README.md`
"Round 4" — the revert leaves this output-network cut geometrically unchanged; the
...
| openEMS FDTD, spliced | **−10.57 dB** | **53.9 GHz** | 43.94 mA (+0.4 %) |
...
Fit note: the vector fit of this touchstone is non-passive by ~3e-3 at every pole
count tried (best (2,14): rms 7.5e-5, worst sampled singular value 1.003144) and
scikit-rf's `passivity_enforce` runs unbounded on it (condition numbers ~1e17,
60+ iterations, seven pole counts tried in parallel for 40 minutes on the
first cut); `em_to_subckt.py --no-enforce --sv-tol 5e-3` skips the enforcement
and accepts the near-passive fit LOUDLY (both flags are new, defaults
unchanged). The touchstone itself is passive, and rung 3 (bias point within
0.4 %) is the check that the excursion is harmless for AC/OP use.
